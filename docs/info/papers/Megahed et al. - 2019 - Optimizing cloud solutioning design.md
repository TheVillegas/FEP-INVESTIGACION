...
the effectiveness and efficiency of our approach as well as its practical potential.
Keywords:
...
5    IT enterprises are shifting from on-premise environments to public, private,
     ...
     deployment models offering more convenient costs, high availability, and flexible
     elasticity.
...
10   ically managed cloud hosting and migration among enterprise customers has
     actually accelerated significantly. In the enterprise marketplace, managed cloud
...
25   ing the existing customer workload, and infrastructure needs and configurations
...
40   level objectives, the most common is cost saving as the result of migrating to
     ...
     problem is a complex and tedious task. This is because given a specific client IT
...
55   finding the optimized combination of solution components, from a cost point of
...
60   IT resources model. In this model, various capabilities are modeled as objects
     with their variations in terms of different acceptable values for any given object’s
...
70   an optimal/minimum cost, from the service provider’s point of view. The key
     ...
     cloud offerings and solution elements over which customized client requirements
...
85      Many enterprises choose to migrate their local IT environment to a cloud
     platform due to its advantage on flexible scalability and low cost. The migration
...
95    thors reported the experience of migrating Hackystat, an Open Source Software
      ...
      ate students to reveal factors impacting end users’ switch to cloud.
         Considering both benefit and risk, many works develop diversified tools and
...
105   respect to different cloud decision objectives. In [14], a general decision pro-
...
115   at reconfiguration of existing systems when migrated to cloud. In [24], Frey et.
      al. show the unmodified system has scalability issues (either under-provision or
...
125   complete scientific computation tasks within time and budget constraints. Other
      works adopt diversified techniques such as evolutionary optimization [26, 27, 28],
...
140      Cloud providers, such as IBM and Amazon, deliver multiple Cloud offerings
...
155   of attributes to formally describes the offering. These attributes may include:
...
170   that describe the considered cloud offering.
      ...
      constraints. The first type concerns the constraints that define sets of combina-
...
180   straints concerns constraints that define sets of combinations of attributes and
...
190      In the general case, solution design may come up with multiple possible
      ...
      difference between these solutions would be the cost/price. In industrial settings,
...
215   code f ∈ F , we define a set Cf for all combinations of attributes and their
      ...
      cost code f ∈ F .
         We define the set N A as the set of combinations of all attributes s ∈ S and
220   their corresponding values v ∈ VS that are not allowed to be in our solution.
      Similarly, we define the set M I as the set of combinations of attributes s ∈ S and
...
225   is, for each element (s, v, s0 , v 0 ) of this set, if solution attribute s ∈ S and its
      corresponding value v ∈ Vs is included in the solution, then attribute s0 ∈ S,
      with its corresponding value v 0 ∈ Vs0 , must be included in the solution, while if
...
230   enables the following logic: For each element (set) in IF , if all attribute and
      ...
      has to always exist last) to enable this logic. Further, set IF N includes the
...
                            Table 1: Sets of Our Optimization Model
       Set Name                               Set Description
           ...
           MI
                     ...
                     Set of quintuples (s, v, s0 , v 0 ) of attributes and their correspond-
                     ing values that must be either included together Or excluded
           IT
                     together in the solution, i.e., either (s, v) and (s0 , v 0 ) are in-
                     ...
                     element i ∈ IF , where i = {(s1 , v1 ), . . . , (si , vi )}, if all pairs
           IF
          ...
          IF N       cluded simultaneously in the solution. Each element i of this
                     set is a set of two or more (attribute, value) pairs (s, v) : s ∈
                     S, v ∈ Vs . |i| is the cardinality of i ∈ IF N
...
      v ∈ Vs for attribute s ∈ S is included in our solution, and zero, otherwise. Vari-
      ...
      Variable cSlacks is 1 if attribute s ∈ S is fulfilled in a customized way, and
...
265   tion, along with a high penalty to minimize the choice of these custom solutions
      ...
      model.
       ...
       Variable
         ...
         Name
                    
                    
                    1, if value v ∈ Vs f or attribute s ∈ S is included in our
                    
                    
                    
           Yvs          solution, and
                    ...
                    
                    
                    1, attribute s ∈ S is f ulf illed in a customized way, and
        cSlacks
                    
                    0, otherwise
                    
      ...
      3.3.3. Model Formulation
         We now present the formulation of our IP optimization model as follows:
                                               ...
                                               12
                         X                                                X
                 Min            costf .Xf + cutomizationP enalty ∗              cSlacks    (1)
                         f ∈F                                             s∈S
                         X
                  s.t.          Yvs = 1, ∀s ∈ S                                            (2)
                         v∈Vs
                         ...
                         Yvs = Yv0 s0 , ∀(s, v, s0 , v 0 ) ∈ IT                            (8)
                                               
                                X
                                          Yvs  − (|i| − 2) ≤ Yvi si , ∀i ∈ IF            (9)
                           (s,v)∈i\{(si ,vi )}
                          X
                                   Yvs ≤ |i| − 1, ∀i ∈ IF N                               (10)
                         (s,v)∈i
...
285   the last one, are included in the solution, then that last one has to be included
      there as well. The logic corresponding to set IF N is captured in constraint
      10, wherein if value v ∈ Vs is included in the solution for attribute s, then
      value v ∈ Vs0 is included in the solution for attribute s0 cannot be included in
...
290   decision variables.
      ...
      gross profit margin to add to this cost in order to come up with the pricing that
...
310   logic/constraints that define valid solutions (correlated to NA set), iii) combi-
      nations of attribute-value pairs that define feature codes with the corresponding
...
      Algorithm 1 Baseline Solution Method 1: Random Selection
       ...
       3: while RemainingAttributes 6= φ do

       4:      Choose a random attribute s0 ∈ RemainingAttributes
       5:      V Vs0 ← {v 0 ∈ Vs0 s.t. the constraints implied by (N A, M I, IT , IF ,
               IF N ) are satisfied given CurrentSolution}
       6:      if V Vs0 = φ then
       7:         RemainingAttributes ← S; CurrentSolution ← φ
       8:         Continue
       9:      Choose a random value v 00 ∈ V Vs0
      10:      CurrentSolution ← CurrentSolution ∪ {(v 00 , s0 )}
      11:      RemainingAttributes ← RemainingAttributes\{s0 }
      ...
      15:         T otalCost ← T otalCost + costf

      16: T otalCost ← T otalCost + customizationP enalty∗ num of attributes not
...
355   (ii) GreedySearch which is a recursive function that constitutes the core of
      the algorithm. The validity of the given solution is checked first. If the solution
...
360   search path. The order of feature code exploration is in ascending order of their
      ...
      not covered by the feature codes in the solution. Finally, the global variable
                                              ...
                                              17
      SolutionF ound is used to exit the algorithm after finding the first feasible so-
...
370   complexity involved in these steps. This is, therefore, one of the strong aspects
      ...
      properties, while still getting a more efficient solution in terms of both cost and
      run-time. Further, such optimal solution could be significantly lower than those
375   obtained by the limited baseline methods that we are presenting here, as we will
      ...
      Algorithm 2 Baseline Solution Method 2: Greedy Heuristic - Part 1
       ...
       4:      CurSol ← φ
       5:      T otCost ← 0
       6:      global : SolutionF ound ← False
       7:      Sort the cost codes in set F in ascending order of cost in costf
       8:      (T otalCost,   Solution)   ←    GreedySearch(CurSol,         RemCodes,
               RemAttr, T otCost)
...
      Algorithm 3 Baseline Solution Method 2: Greedy Heuristic - Part 2
       ...
       1: procedure GreedySearch(CurrSol, RemCodes, RemAttr, T otCost)

       2:      if global:solutionF ound then
       3:          return
       4:      if CheckConstraints(CurrSol,N A, M I, IT , IF , IF N ) = False then
       5:          return
       6:      for f ∈ RemCodes (where we iterate on the sorted set F ) do
       7:          CurrSol0 ← CurrSol ∪ {(v, s) ∈ Cf }
       8:          RemCodes0 ← RemCodes\{f }
       9:          RemAttr0 ← RemAttr\{s ∈ Cf }
      10:          T otCost0 ← T otCost + costf
      11:          GreedySearch(CurrSol0 , RemCodes0 , RemAttr0 , T otCost0 )

      12:      global:solutionF ound ← True
      13:      return CustomizeRemainingAttributes(RemAttr,T otCost,CurrSol)
...
390   but none of them proved to be significantly different from the default settings.
...
      Algorithm 4 Baseline Solution Method 2: Greedy Heuristic - Part 3
       ...
       1: procedure CustomizeRemainingAttributes(RemAttr, T otCost, CurrSol)

       2:      RemAttr0 ← RemAttr
       3:      T otCost0 ← T otCost
       4:      CurrSol0 ← CurrSol
       5:      while RemAttr0 6= φ do
       6:         Choose a random attribute s0 ∈ RemAttr0
       7:         V Vs0 ← {v 0 ∈ Vs0 s.t. the constraints implied by (N A, M I, IT , IF ,
                  IF N ) are satisfied given CurrSol0 }
       8:         if V Vs0 = φ then
       9:            RemAttr0 ← RemAttr
      10:            T otCost0 ← T otCost
      11:            CurrSol0 ← CurrSol
      12:            Continue
      13:         Choose a random value v 00 ∈ V Vs0
      14:         CurrSol0 ← CurrSol0 ∪ {(v 00 , s0 )}
      15:         RemAttr0 ← RemAttr0 \{s0 }
      16:         T otCost0 ← T otCost0 + CustomizationP enalty
               return T otCost0 ,CurrSol0
...
400   Second, we compare the efficiency of our optimization approach compared to
...
410   IF and IF N . We solved all the instances with the constraints implied by (IF ,
      IF N ) and without these constraints. All of our experiments were run on a 2.1
...
420   implied by (IF , IF N ). From the figures, we can note that following:
         ...
         • Comparing the Optimization Model to the Greedy Heuristic:
              ...
              – The absence of the constraints implied by (IF , IF N ) amplifies the
425             cost savings obtained by the optimization model.

              – In the absence of the constraints implied by (IF , IF N ), the opti-
                mization model is slightly slower than the greedy heuristic.

              – In the existence of the constraints implied by (IF , IF N ), the opti-
...
435           – The constraints implied by (IF , IF N ) has no signifcant effect on
...
440   results, one can obviously tell the effectiveness of our optimization method given
...
465   ods and thus, does not incorporate any additional effort in that regard. Finally,
...
475   ods and compared the results of applying all three methods on realistic data.
      Experimentation results show the effectiveness of our optimization method since
...
485      Given the aforementioned effectiveness and efficiency of our approach, as
...
490   to find the minimum possible solution that satisfied the client requirements.
      ...
      ”price” their solution. That is, they need to add some gross profit on top of the
...
520       tion performance in the cloud, in: Proceedings of the first annual ACM
...
545   [14] P. V. Beserra, A. Camara, R. Ximenes, A. B. Albuquerque, N. C. Men-
          donça, Cloudstep: A step-by-step decision process to support legacy appli-
...
570       and Distributed Computing 96 (2016) 75–94.
      ...
      [22] T. Binz, U. Breitenbücher, O. Kopp, F. Leymann, Tosca: Portable auto-
...
580       of software systems to cloud-optimized applications, International Journal
      ...
      [25] V. Andrikopoulos, S. G. Sáez, F. Leymann, J. Wettinger, Optimal distribu-
