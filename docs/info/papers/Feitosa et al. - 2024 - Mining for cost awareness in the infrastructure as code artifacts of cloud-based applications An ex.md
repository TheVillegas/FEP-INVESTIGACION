...
Mining software repositories
                                                            Objective: This study aims to provide empirical evidence of cost awareness by mining open source repos-
Cloud orchestration
...
to collect evidence of this.                                                        and researchers, including the formulation of a research agenda for
    The objective of this study is therefore clear: to examine to what              future work. Finally, Sections 7 and 8 close this paper with a reflection
extent software developers are aware of the cost of deploying and operating         on the threats to validity to this study, and a summary of its main
...
is usually achieved by means of descriptor files, i.e., configuration files         which cloud service provider(s) to use (Hosseinzadeh et al., 2020).
that when interpreted by the orchestrator ensure that both the under-               This also appears to be the case for the related problem of optimizing
lying infrastructure is made available, and the tasks required for the              the selection of services from potentially across service providers, com-
(re)deployment of software on this infrastructure is executed correctly.            monly known as cloud service composition (Amato and Venticinque,
...
development, starting with IaC artifacts. In addition, we fortify the
                                                                                      1
findings of the mining process by also investigating to what extent the                   https://stackoverflow.com/
...
orchestrators (de Carvalho and de Araujo, 2020). Terraform is known               3
                                                                                    https://cloudify.co/
for providing an open-source version, cloud services compatibility,               4
                                                                                    https://www.terraform.io/language/files
interface accessibility and mature API (de Carvalho and de Araujo,                5
                                                                                    https://pygithub.readthedocs.io/
2020).                                                                            6
                                                                                    This was done to avoid the limitations in the amount of results by the
                                                                                GitHub API per request.
                                                                                  7
                                                                                    Treating them as stems means that expens for example will match
  2
      https://www.terraform.io/                                                 against expense expenses, inexpensive, and inexpensively.
...
that cost may be a concern for the project.                                       We also remove tokens according to a stop-word list that we built by
    To extract the issues, we provide GrimoreLab’s Perceval (Dueñas               merging the Gensim’s (Řehůřek and Sojka, 2010) list for English with
et al., 2018) with the repository owner username, the repository name,            terms deemed irrelevant for our analysis. We perform the same steps
and a GitHub API token. Perceval is a Python-based tool that can collect          for both commits’ and issues’ text.
data from various sources, including issue trackers. We chose Perceval                With the prepared corpus, we use Gensim to convert it into a bag
for its versatility and validation within the MSR community. The tool             of words, build a TF-IDF (term frequency–inverse document frequency)
then returns us a list of issue objects that contain every single detail          model and use the two to create LDA models. To build an LDA model,
pertaining the issue. We then extract the issue objects that contain              we must find a suitable number of topics (𝐾), as it impacts the gran-
one or more of the defined keywords in the title, body or any of the              ularity of the results (Abdellatif et al., 2020; Han et al., 2020). The
comments; this results in an initial set of 862 entries.                          quality of the model is also affected by other hyperparameters, such as
    Next, we apply a process similar to that described for RQ1 to                 𝛼 (referring to document-topic density) and 𝛽 9 (referring to topic-word
filter for relevant issues and then label them. Since issues may con-             density) are some of the most relevant (Campbell et al., 2015; Treude
...
(only 44%). Nevertheless, the descriptive statistics summarized by
                                                                                  coding performed for RQ1 and RQ2 as well as from the interpretation of
Fig. 3 suggest a fair distribution of data points, with higher averages
...
(by approx. a factor of two).
    ...
    Similar to RQ1 , the collected units are analyzed to identify rele-
                                                                                  analyzed the top words of a topic via a random sample of documents
...
compared to those obtained from RQ1 , to reflect on the value added
                                                                                  of documents connected to a given topic (we aimed for 5%–10% of
...
3.3.3. RQ3                                                                          8
   ...
   In the final research question, we want to examine the dataset in                9
                                                                                      In Gensim, the parameter ‘eta’ refers to 𝛽.
                                                                                   10
more depth and make contextual connections between the identified                     https://radimrehurek.com/gensim/models/ldamodel.html
...
4.1. Cost Awareness in Commits (RQ1 )
                                                                                      less prevalent, we also identified information explicitly discussing cost
...
elaborate on the most recurrent ones in the following.
    The most popular label by all metrics is saving. A notorious
                                                                                       11
example from the dataset is:                                                                See e.g. https://twitter.com/quinnypig/status/1440301033314349062
...
is more recurrent than saving among issues, which might be related
                                                                                 labels are represented as columns on the top part and described through
...
or acknowledging cost increase. The example below depicts a situation            of this process, we created the knowledge graph depicted in Fig. 8.
of a seemingly unintentional increase.                                           The graph compiles three main levels of information: effects on cost,
...
5.1. Data collection
                                                                                            13
                                                                                               https://archive.org/download/stack-exchange-data-dump-2023-09-
   The data collection entailed the extraction of relevant discussions
                                                                                           12/stackoverflow.com-Posts.7z
from SO. For that, we used key datasets from the September 12,                              14
                                                                                               https://archive.org/download/stack-exchange-data-dump-2023-09-
                                                                                           12/stackoverflow.com-Comments.7z
                                                                                            15
                                                                                               https://archive.org/download/stack-exchange-data-dump-2023-09-
 12
      https://stackoverflow.com/                                                           12/stackoverflow.com-PostHistory.7z
...
services or asking for advice in this direction:                                          These combined observations show that the knowledge graph cannot
      ...
      a new version.
      post id: #72881767                label: storage                                      16
...
academic researchers. With respect to the former, it becomes clear that                 more attractive sources of data.
cost awareness should actually be present, if it is not already, through-             ➥ Apply natural language processing and other machine learning
...
Stack Overflow on the topic, and the corresponding answers, it becomes               17
                                                                                          https://docs.oasis-open.org/tosca/TOSCA/v1.0/TOSCA-v1.0.html
                                                                                     18
clear that having such knowledge in advance at their disposal would                       https://aws.amazon.com/cloudformation/
...
our data sets (from GitHub and Stack Overflow) may not represent
                                                                                   same repositories (RQ2 ) did not reveal any additional information in
all possible cost-related discussions in cloud projects. The inclusion of
repositories hosted in other (closed-source) platforms, or that use other          terms of identified topics of discussion. It did however offer further
cloud orchestrators, or that use no cloud orchestrators, could lead to             insights into the decision-making process entailed in managing cloud
the identification of new discussions. The inclusion of discussion forums          costs that can be pursued further in future work.
...
during the investigation of RQ3 , which was part of the axial coding
                                                                                   ization. Matei-Tudor Penca: Software, Formal analysis, Data cura-
...
cost-related discussion of Terraform-based deployments. The mitigation             Methodology, Conceptualization.
strategies in this case entail the use of Terraform-related tags that have
been assigned by users, on top of using previously-validated keywords              Declaration of competing interest
